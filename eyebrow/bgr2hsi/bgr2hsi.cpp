...
vector<uchar> BGR2HSI::transformationMap(const vector<double>& histogram)
{
    ...
    vector<uchar> transformation_map(L, 0);

    double cumulative_frequency_sum = 0.0;
    for(int i = 0; i < L; ++i)
    {
        cumulative_frequency_sum += histogram[i];
        transformation_map[i] = round(cumulative_frequency_sum * (L-1));
    }
    
    return transformation_map;
}

Mat BGR2HSI::equalizedImage(const Mat& intensity_plane, const vector<uchar>& transformation_map)
{
    Mat equalized_intensity_plane(intensity_plane.size(), CV_8UC1);
    for(int i = 0; i < intensity_plane.rows; ++i)
    {
        const uchar* _intensity_plane_row = intensity_plane.ptr<uchar>(i);
        uchar* _equalized_intensity_plane_row = equalized_intensity_plane.ptr<uchar>(i);
        
        for(int j = 0; j < intensity_plane.cols; ++j)
            _equalized_intensity_plane_row[j] = transformation_map[_intensity_plane_row[j]];
    }
    return equalized_intensity_plane;
}

vector<double> BGR2HSI::equalizeHistogram(const Mat& equalized_intensity_plane)
{
    return calculateHistogram(equalized_intensity_plane);
}
